...
export abstract class BaseAgent extends Construct {
  ...
  constructor(scope: Construct, id: string, props: BaseAgentProps) {
    ...
    // Create retrieval tool asset if knowledge bases are configured
    if (knowledgeBases.length > 0) {
      this.knowledgeBaseToolAsset = new Asset(this, 'KnowledgeBaseRetrievalTool', {
        path: path.join(__dirname, 'resources/knowledge-base-tool'),
      });

      // Grant read access to the retrieval tool asset
      this.knowledgeBaseToolAsset.grantRead(this.agentRole);

      // Add retrieval tool to agent tools location definitions
      this.agentToolsLocationDefinitions.push({
        bucketName: this.knowledgeBaseToolAsset.s3BucketName,
        key: this.knowledgeBaseToolAsset.s3ObjectKey,
        isFile: this.knowledgeBaseToolAsset.isFile,
        isZipArchive: this.knowledgeBaseToolAsset.isZipArchive,
      });
    }
