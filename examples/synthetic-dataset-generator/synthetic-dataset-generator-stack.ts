...
export class SyntheticDatasetGeneratorStack extends Stack {
  ...
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    // =========================================================================
    // Session Lock Table - Distributed Locking for Concurrent Request Prevention
    // =========================================================================

    // DynamoDB table for distributed session locking across Lambda instances.
    // Prevents race conditions when multiple requests hit the same session.
    const sessionLockTable = new Table(this, 'SessionLockTable', {
    ...
    this.chatAgent = new InteractiveAgent(this, 'ChatAgent', {
      ...
      corsAllowMethods: ['GET', 'POST', 'DELETE', 'PATCH', 'OPTIONS'],
      agentDefinition: {
        bedrockModel: {
          useCrossRegionInference: true,
        },
    ...
    // Session lock table for distributed locking
    cfnFunction.addPropertyOverride(
      'Environment.Variables.SESSION_LOCK_TABLE',
      sessionLockTable.tableName,
    );

    // Grant the chat agent Lambda permission to use the lock table
    sessionLockTable.grantReadWriteData(this.chatAgent.agentFunction!);

    // Grant KMS decrypt permission for presigned URL generation
    // The chat agent needs kms:Decrypt to generate working presigned URLs for the KMS-encrypted export bucket
    exportBucketKey.grantDecrypt(this.chatAgent.agentFunction!);

    // =========================================================================
    // History API - Separate Lambda (no Lambda Web Adapter)
    // =========================================================================

    // Create a simple Lambda for /history and /sessions endpoints
    // This avoids Lambda Web Adapter issues with standard API Gateway proxy integration
    const historyLambda = new PythonFunction(this, 'HistoryHandler', {
      functionName: 'DataSynthHistory',
      ...
      memorySize: 256,
      environment: {
        SESSION_BUCKET: this.chatAgent.sessionBucket!.bucketName,
        // Use the session index table from the InteractiveAgent construct
        ...this.chatAgent.sessionIndex!.environmentVariables(),
      },
    });

    // Grant read access to session bucket (for chat history)
    this.chatAgent.sessionBucket!.grantRead(historyLambda);

    // Grant read/write access to session index table (for listing/deleting sessions)
    this.chatAgent.sessionIndex!.grantReadWrite(historyLambda);

    // Add routes to existing API Gateway
    if (this.chatAgent.restApi && this.chatAgent.cognitoAuthorizer) {
      const historyIntegration = new LambdaIntegration(historyLambda);

      // /history/{session_id} - GET chat history for a specific session
      const historyResource = this.chatAgent.restApi.root.addResource('history');
      const sessionIdResource = historyResource.addResource('{session_id}');
      sessionIdResource.addMethod('GET', historyIntegration, {
        authorizationType: AuthorizationType.COGNITO,
        authorizer: this.chatAgent.cognitoAuthorizer,
      });

      // /sessions - GET list of all sessions for the authenticated user
      const sessionsResource = this.chatAgent.restApi.root.addResource('sessions');
      sessionsResource.addMethod('GET', historyIntegration, {
        authorizationType: AuthorizationType.COGNITO,
        authorizer: this.chatAgent.cognitoAuthorizer,
      });

      // /sessions/{session_id} - DELETE a specific session
      const sessionIdForDeleteResource = sessionsResource.addResource('{session_id}');
      sessionIdForDeleteResource.addMethod('DELETE', historyIntegration, {
        authorizationType: AuthorizationType.COGNITO,
        authorizer: this.chatAgent.cognitoAuthorizer,
      });

      // /sessions/{session_id} - PATCH to rename a session
    ...
    // Session index table name (for debugging)
    new CfnOutput(this, 'SessionIndexTableName', {
      value: this.chatAgent.sessionIndex!.environmentVariables().SESSION_INDEX_TABLE,
      description: 'DynamoDB table for session index',
    });
