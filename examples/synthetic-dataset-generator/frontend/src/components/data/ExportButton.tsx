...
export function ExportButton({ downloads, disabled = false }: ExportButtonProps) {
  ...
  const hasDownloads = downloads && (
    downloads.csv_url || downloads.json_url || downloads.schema_url || downloads.script_url
  );
  ...
  const options = [
    { key: 'csv_url' as const, label: 'CSV', icon: FileText },
    { key: 'json_url' as const, label: 'JSON', icon: FileJson },
    { key: 'schema_url' as const, label: 'Schema', icon: FileText },
    { key: 'script_url' as const, label: 'Generation Script', icon: FileCode },
  ];
