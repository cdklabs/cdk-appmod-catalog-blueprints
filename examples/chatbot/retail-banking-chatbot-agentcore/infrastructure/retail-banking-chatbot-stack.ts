...
export class RetailBankingChatbotStack extends Stack {
  constructor(scope: Construct, id: string, props?: StackProps) {
    ...
    // =========================================================================
    // Cognito User Pool (for AgentCore JWT auth)
    // =========================================================================
    ...
    // Cognito OIDC discovery URL for AgentCore JWT authorizer
    const cognitoIssuerUrl = `https://cognito-idp.${this.region}.amazonaws.com/${userPool.userPoolId}`;

    // =========================================================================
    // CloudWatch Transaction Search (prerequisite for AgentCore observability)
    // =========================================================================

    new CloudWatchTransactionSearch(this, 'TransactionSearch');

    // =========================================================================
    // Interactive Agent — AgentCore Runtime hosting with Cognito auth
    // =========================================================================
    ...
    const agent = new InteractiveAgent(this, 'BankingAgent', {
      ...
      servicePrincipal: new ServicePrincipal("bedrock-agentcore.amazonaws.com")
    });

    agent.node.addDependency(ingestionTrigger);

    // Pass DynamoDB table name to the agent container
    agent.agentRole.addToPrincipalPolicy(new PolicyStatement({
      effect: Effect.ALLOW,
      actions: ['dynamodb:Query', 'dynamodb:GetItem'],
      resources: [transactionsTable.tableArn],
    }));
    if (agent.cfnRuntime) {
      agent.cfnRuntime.addPropertyOverride(
    ...
    new CfnOutput(this, 'AgentCoreEndpoint', {
